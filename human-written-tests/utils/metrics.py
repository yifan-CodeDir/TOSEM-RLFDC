...
def TfD_network(X, x=None, **kwargs):
    if x is None:
        return .0
...
def Prox(X, x=None, **kwargs):
    ...
    y = kwargs['y']
    prox = cdist(X[y==0], torch.unsqueeze(x, 0), 'jaccard') # calculate Jaccard distance, =1-coeff
    return(1.-float(np.mean(prox[:, 0])))  # calculate coeff, more similar, the bigger
...
def TfD_pro2(X, x=None, **kwargs):   # revise target, to only consider the statements covered by failing tests
    if x is None:
        return .0
    ...
    for j in range(unique.shape[1]):  # calculate values for each ambiguity group
        ...
        div = min(int(torch.sum(x[group_index] == 0)), int(torch.sum(x[group_index] == 1))) 
        spilt += div * group_prio
    return float(alpha * cover + (1 - alpha) * spilt)


def TfD_pro3(X, x=None, **kwargs):   # revise target, to only consider the statements covered by failing tests
    if x is None:
        return .0
    ...
    # calculate cover, i.e., the jaccard coeff with initial failing test
    cover = 1 - np.mean(cdist(X[[0], :], torch.unsqueeze(x, 0), 'jaccard')) 
    susp_elems_num = int(torch.sum(X[0, :]))
    _, n = X.shape

    # calculate spilt
    # target = kwargs['target']
    target = X[0,:] > 0    # only consider those covered by failing tests
    X = X[:, target]
    x = x[target]
    ...
    for j in range(unique.shape[1]):  # calculate values for each ambiguity group
        group_index = (indices == j)
        group_prio = torch.sum(group_index) / n  # |g|/n, normalize
        div = min(int(torch.sum(x[group_index] == 0)), int(torch.sum(x[group_index] == 1))) / susp_elems_num
        spilt += div * group_prio
    return float(alpha * cover + (1 - alpha) * spilt)

def TfD_pro3_cover(X, x=None, **kwargs):   # revise target, to only consider the statements covered by failing tests
    if x is None:
        return .0
