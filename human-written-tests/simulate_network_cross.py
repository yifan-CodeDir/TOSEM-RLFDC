...
if __name__ == "__main__":
    ...
    time_record_df = pd.DataFrame(index=["training_time"],columns=["Chart","Lang","Math","Closure","Time"])
    if is_train:     # if train, need to train and save model
        
        for p in train_projects:
            save_model_path=f"network_model_v5_cross_{p}"     
            ...
            time_record_df.loc["training_time"][p] = train_end_time - train_start_time

        # # record the training time of each project in [Chart folder]
        # time_record_df.to_csv(f"./network_model_v6_cross_Chart/{train_projects}_time_record.csv")

    for train_p in train_projects:  # evaluate
        # load model from the corresponding directory
        # test_start_time = time.time()
        load_model_path=f"network_model_v5_cross_{train_p}"
        model = torch.load(f"./{load_model_path}/{train_p}.pt") 
        # start = 5
        # end = 25
        output_path = f"./network_model_v5_cross_{train_p}/RLFDC_train_on_{train_p}.json"
        simulation_results = {}
        
        for p in projects:
            
            if p == train_p:  # skip the train project
                continue
            start = projects[p][0]
            end = projects[p][1]
