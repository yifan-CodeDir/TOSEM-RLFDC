...
def loop(project, version, tool, ts_id, selection_metric_name, failing_test_name, select_all,
  budget=10, noise_prob=0.0, new=False, skip_broken=False, verbose=False):
  env = EvoD4jEnv(project, version, ts_id, failing_test_name)
  ...
  if os.path.exists(env.covmat_path) and os.path.exists(env.oravec_path):
    ...
    for m in buggy_methods:
      print("--- {}".format(m))
    print("* failing test: {}".format(len(failing_test_name)))
      ...
      with open(env.oracle_path, 'r', encoding='utf-8') as f:
        for l in f:
          if l.startswith("--- "):
            ...
            # test_no = int(test_no[4:])  # FIXME: fix this bug 
    ...
    """
    Collect the coverage of the specific failing tests, generated tests)
    """
    lines = set()   
    coverage_path = os.path.join(env.coverage_dir, failing_test_name + ".xml") # collect failing test coverage 
    lines.update(get_hits(coverage_path)['lid'].values.tolist())

    print("* {} lines collected".format(len(lines)))
    for coverage_file in os.listdir(env.evosuite_coverage_dir): # collect generated tests coverage 
    ...
    coverage_path = os.path.join(env.coverage_dir, failing_test_name + ".xml")
    cov_vector = cov.generate(get_hits(coverage_path)['lid'].values.tolist())
    tests.append(
      TestCase("initial_test", failing_test_name, cov_vector, None, TestResult.FAILING)
    )
  ...
  if noise_prob == 0.0:
    ...
    tests_path = os.path.splitext(env.tests_path)[0] + f"_noise_{noise_prob}.pkl"  # 分割后缀名
    method_ranks_path = os.path.splitext(env.ranks_path)[0] + f"_noise_{noise_prob}.pkl"

  tests_path = tests_path.replace("tests", "tests-{}-{}".format(selection_metric_name, budget))
  method_ranks_path = method_ranks_path.replace("ranks", "ranks-{}-{}".format(selection_metric_name, budget))
  # table_path = tests_path.replace("tests-{}".format(selection_metric_name), "record-{}".format(selection_metric_name))

  # select tests and produce results, if already exists, directly output them
  ...
  #   print(tests)
  # else:
  print("********************************************") # for experiment, we may repetively do localization
  if select_all:
    budget = coverage_matrix.shape[0] - 1   
  
  tests, method_ranks = select(project, version, coverage_matrix, oracle_vector, buggy_methods,
...
def select(project, version, coverage_matrix, oracle_vector, buggy_methods,
           selection_metric, budget=10, noise_prob=0.0, metric_params={},
           skip_broken=False, verbose=False):

  result_vector = oracle_vector.copy()
  result_vector[[tc[0] != "initial_test" for tc in result_vector.index]] = TestResult.NEEDASK.value  # reset oracle to simulate iterative fault localization， mask all tests' oracle except for the initial failing test
  ...
  rows = []
  # table = pd.DataFrame(columns=["Iter", "Test", "Fitness", "Oracle", "Response", "Ranks"])  # record results
  i = 0
  while True:
    s_covmat, s_resvec, is_valid = slice_data(coverage_matrix, result_vector)  # select those oracle value != NEEDASK
    ...
    X = torch.tensor(s_covmat.values)  # the coverage matrix of the selected tests
    ...
    method_ranks[f"rank-{i}"] = method_ranks['method'].map(mr_map)  # record rank and score for each iteration
    ...
    # record in the table
    # tmp = pd.Series(row, index=table.columns)
    # table.append(tmp, ignore_index=True)
    ...
    model = torch.load(f"./model/{save_model_path}/{project}.pt", map_location=torch.device('cpu')) 

    # calculate state (i.e. input for network)
    N = len(coverage_matrix)
    state = getState(X)   # input:current_cov_matrix
    state_embed = model.encoder(state)
    state_embed_re = state_embed.repeat(N,1)  # (N,16)
    
    all_X = torch.tensor(coverage_matrix.values)   # coverage matrix for all the tests
    all_feature = getallFeature(X, all_X)
    fitness = model.eval_net(torch.cat([all_feature, state_embed_re], dim=1))   # (N,1)
    mask = torch.tensor(is_valid).reshape(-1, 1)
    fitness[mask] = -math.inf # mask fitness of the selected tests
    # TODO: construct fitness list
    all_test_name = coverage_matrix.index.values
    sort_fitness_df = pd.DataFrame()
    sort_fitness_df["fitness"] = fitness.squeeze(1).detach().numpy()
    sort_fitness_df["test_name"] = all_test_name

    # calculate fitness one by one, we want to parelize the process
    ...
    #     continue
    #   if selection_metric != TfD_network:
    ...
    #     state = getState(X) 
    #     state_embed = model.encoder(state)
    #     feature = getFeature(X, x)
    #     fitness = model.eval_net(torch.cat([feature, state_embed], dim=0))  
    #   fitnesses.append((fitness, test, j))

    sort_fitness_df = sort_fitness_df.sort_values(by="fitness", ascending=False)
    if len(sort_fitness_df) > 0:
      selected_fitness, selected_test = sort_fitness_df.iloc[0]
      selected_index = sort_fitness_df.index[0]
    else:
      break
    if verbose:
...
if __name__ == "__main__":
  ...
  '''
  TODO: Get failing test for each project-version
  '''
  failing_tests_file = os.path.join(
          os.environ['D4J_METADATA'], "{}-{}".format(args.project, args.version), "tests.trigger")
  ...
  """
  # for each test suite formed by each failing test, do fault localisation 
  for failing_test in failing_tests:
    loop(args.project, args.version, args.tool, args.id, args.selection, failing_test, args.select_all,
