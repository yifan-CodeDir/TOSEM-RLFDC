...
if __name__ == "__main__":
  ...
  parser.add_argument('failing_test', type=str)
  ...
  env = EvoD4jEnv(project, version, ts_id, failing_test)
  ...
  for dp, dn, fn in os.walk(env.extracted_dir):
    for f in fn:
      if f.endswith("ESTest.java"):
        ...
        for test in sorted(coverage.keys()):  # test00, test01, ...
  ...
  while True:
    ...
    print("defects4j test -s {}".format(env.minimised_suite_file))
    cp = subprocess.run(
