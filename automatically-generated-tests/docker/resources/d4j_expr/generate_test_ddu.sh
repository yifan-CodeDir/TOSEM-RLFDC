    ...
    for class in $(cat $RELEVANT_CLASSES); do      # generate test for each relevant classes
      echo $class
        if [ -f "$RELEVANT_METHODS_DIR/$class" ]; then
          ...
          echo ""
          echo "-----------------------------------------------"
          echo "$EVOSUITE -class $class -projectCP $project_cp -Dstrategy=DDU_SINGLE -Dcriterion DDU -generateTests -Dsearch_budget=$class_budget -Dstopping_condition=MaxTime -Dbudget_for_each_generation=2 -seed=$SEED -Dreport_dir=$EVOSUITE_REPORT/${ID}/${PROJECT}-${VERSION} -Dtest_dir=${TEST_SUITE_CONTENT}/${tc} $(cat ${EVOSUITE_CONFIG})"
          ...
          $EVOSUITE -class $class -projectCP $project_cp -Dstrategy=DDU_SINGLE -Dcriterion DDU -generateTests -Dsearch_budget=$class_budget -Dstopping_condition=MaxTime -Dbudget_for_each_generation=2 -seed=$SEED -Dreport_dir=$EVOSUITE_REPORT/${ID}/${PROJECT}-${VERSION}/ -Dtest_dir=${TEST_SUITE_CONTENT}/${tc} $(cat ${EVOSUITE_CONFIG})
