...
class EvoD4jEnv:
    ...
    EVOSUITE_COVERAGE = os.environ['EVOSUITE_COVERAGE'] # /root/results/evosuite_coverage/
    EVOSUITE_ORACLE = os.environ['EVOSUITE_ORACLE']   # /root/results/evosuite_oracles/
    EVOSUITE_FL = os.path.join(D4J_EXPR_RESULTS, "localisation")
    TMP_ROOT = '/tmp'

    def __init__(self, project, version, ts_id, failing_test):
        cls = self.__class__
        ...
        self.extracted_dir = os.path.join(self.evosuite_test_dir, self.d4j_id, self.failing_test) #################
        self.minimised_dir = os.path.join(self.evosuite_test_dir, self.d4j_id + "-minimised", self.failing_test) #################
        self.minimised_suite_file = os.path.join(self.evosuite_test_dir, self.d4j_id + "-minimised", self.d4j_id + "-minimised.tar.bz2") #################
        self.evosuite_coverage_dir = os.path.join(self.EVOSUITE_COVERAGE, self.ts_id, self.d4j_id, self.failing_test) #################

        self.oracle_dir = os.path.join(cls.EVOSUITE_ORACLE, self.ts_id, self.d4j_id, self.failing_test) #################
        if not os.path.exists(self.oracle_dir):
          os.makedirs(self.oracle_dir)

        self.oracle_path = os.path.join(self.oracle_dir, "{}.{}.failing_tests".format(self.d4j_id, self.failing_test)) #################
        self.broken_test_path = os.path.join(self.oracle_dir, "{}.{}.broken_tests".format(self.d4j_id, self.failing_test)) #################

        if not os.path.exists(cls.EVOSUITE_FL):
          os.mkdir(cls.EVOSUITE_FL)
        self.localisation_dir = os.path.join(cls.EVOSUITE_FL, self.ts_id, self.d4j_id, self.failing_test)  #################
