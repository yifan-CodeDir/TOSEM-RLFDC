...
if [ -d "$TEST_SUITE_CONTENT" ]; then               # generate tests with EVOSUITE
  ...
  do
    ...
    for class in $(cat $RELEVANT_CLASSES); do      # generate test for each relevant classes
      ...
      $EVOSUITE -measureCoverage -class $class -Djunit=${tc/::/#} -criterion TFDNET -projectCP ${project_cp}:${BUGGY_TMP_DIR}/${test_relat_cp} -Dreport_dir=$EVOSUITE_REPORT/${ID}/${PROJECT}-${VERSION}/ -Dcoverage_matrix=true -DmeasureSingleCoverage=true # measure coverage using EVOSUITE, $EVOSUITE_REPORT=/root/results/evosuite_report/
      if [ -s $EVOSUITE_REPORT/${ID}/${PROJECT}-${VERSION}/data/${class}.TFDNET.matrix ]; then  # if the coverage file is not empty
