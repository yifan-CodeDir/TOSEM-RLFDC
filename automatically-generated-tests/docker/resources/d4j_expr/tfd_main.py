...
def select(project, version, coverage_matrix, oracle_vector, buggy_methods,
  ...
  result_vector[[tc[0] != "initial_test" for tc in result_vector.index]] = TestResult.NEEDASK.value  # reset oracle to simulate iterative fault localization
  ...
  i = 0
  while True:
    s_covmat, s_resvec, _ = slice_data(coverage_matrix, result_vector)  # select those oracle value != NEEDASK
